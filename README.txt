...
Unique visitors (by IP), total hits and hits per visitor, per day, at https://piers.qa/krill/stats/ - a table, newest day first. build.py writes that page into _site/stats/ with no beacon, and the browser fills it from https://krill-hits.piers.qa/stats.json, which only https://piers.qa may read from script - so opened from a local file it says it could not load the counts. The old https://krill-hits.piers.qa/stats redirects to it. A Worker route on the piers.qa path itself would have needed Cloudflare to proxy the whole domain, /gigs included. GitHub Pages keeps no access logs and piers.qa is DNS-only on Cloudflare, so neither sees the traffic - hence a separate Worker.

Every full page ends with a sendBeacon POST to https://krill-hits.piers.qa/hit. The Worker in counter/ rejects anything whose Origin is not https://piers.qa, hashes date + IP + a secret salt, and upserts it into a D1 table keyed on (day, visitor). An IPv6 visitor is keyed by its /64 rather than its full address, because Windows and phones rotate temporary IPv6 addresses within their /64 and counting whole addresses split one PC into several visitors in a day. In the table, the first hit of the day adds a row, every later one from the same IP bumps that row's hits. A hit is a page load that runs the script - archive navigation and the root page's stale-day reload count, a back-button return from the browser's page cache does not. Days are UTC calendar days. Raw IPs are never stored, and the date in the hash means one visitor cannot be tracked across days, so hits per visitor is a list of counts with no addresses attached. The artifact (--fragment) build carries no beacon, since its host blocks it by CSP.
