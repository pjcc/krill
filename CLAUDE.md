...
- **A Windows console is cp1252, so an unmappable character in a progress line kills the run.** `fetch.py` reconfigures `stdout`/`stderr` to `errors='replace'` at import. Without it, printing a resolved title like 'Kanga pirau' (macron) raises `UnicodeEncodeError` and aborts after hundreds of throttled requests - it took out a `reenrich.py` pass midway through the archive on 2026-09-14. Logging must never be able to abort a capture
...
`rest.php/v1/search/title` is a **fuzzy completion, not a lookup**, which is why it misfired so widely: 'Set' offers Seth MacFarlane and Set theory, 'Oca' offers Alexandria Ocasio-Cortez and Ric Ocasek. `fetch.names()` now gates every search hit - a hit counts as the answer only when its title *is* the answer, allowing for case, accents, punctuation, word spacing and a trailing Wikipedia qualifier (`Set (card game)`, `Rough and Ready, California`). Anything that merely starts with the answer or extends it by a word is dropped, and an answer with no plausible hit shows **no article** rather than a confident wrong one. The gate compares the *requested* title, not the article the summary redirects to, so genuine redirects still resolve (Pistol shrimp -> Alpheidae).

Of the 40 answers in the archive that came from the search rather than the literal title, 16 were wrong and the gate removes them. It cannot judge **sense**, only name: 'Asterism' on a punctuation prompt still matched 'Asterism (astronomy)', 'Blob' on a mass prompt matched the Marvel character, 'Pippin' on an Apple prompt matched the musical. Same-name-wrong-sense stays an `overrides.json` job. Three correct links were collateral ('Maltese' -> Maltese language, 'Nabla' -> Nabla symbol, 'Sauce nantua' -> Nantua sauce) and are pinned there too. The stem fallback also now skips a stem of nothing but articles and prepositions, which is what sent 'The Maschinenmensch' to the article 'The'.

The **prefix fallback** - dropping trailing words until something has an article - is the other source of wrong hits: 'Khai luk khoei' fell back to Khai, a village in Punjab, and 'Jib door' to the sail. `fetch.explains()` keeps a stem's article only if its description or extract mentions every dropped word (bar stopwords, allowing a plural): Latia's says limpet, Cochineal's says insect. On the 10 prefix matches in the archive to 2026-09-23 it kept those two and Spanish dancer, all right, and dropped 7, all wrong or loose. A kept one renders the tag 'partial match: <title>'. `reenrich.py` re-resolves every `approx` item as well as every override, so a gate change applies to the archive in one run.

**Do not raise the search `limit` above 4 to find more candidates** - it makes resolution worse, not better. A parenthetical hit proves the name matches, never the sense, and the first passing hit wins: at `limit=10` 'Set' reaches 'Set (mathematics)', 'Dalmatian' reaches 'Dalmatian (EP)', 'Nabla' reaches 'Nabla (moth)' and 'Yamato' reaches 'Yamato (film)', none of which appear in the top 4.
...
Unique visitors (by IP), total hits and hits per visitor per UTC day, read at **https://piers.qa/krill/stats/**. That page is static - `build.stats_doc()` writes `_site/stats/index.html`, with no beacon - and the browser fills it from `https://krill-hits.piers.qa/stats.json`, which sends `Access-Control-Allow-Origin: https://piers.qa`. Serving the path from the Worker instead would have needed a Worker route, which needs `piers.qa` proxied by Cloudflare - all of it, `/gigs` included, with SSL-mode and Pages certificate-renewal risk. The Worker's old `/stats` 301s to the page. The stats are public by the user's choice. GitHub Pages has no access logs, and `piers.qa` is **DNS-only** on Cloudflare (A records point at GitHub), so Cloudflare's zone analytics never see the traffic either. Proxying the zone to get them would cover all of `piers.qa`, not just `/krill`, so the counter is a separate Worker instead.
